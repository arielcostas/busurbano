...
export const StopSheet: React.FC<StopSheetProps> = ({ isOpen, onClose, stopId, stopName }) => {
  ...
  const limitedEstimates = data?.estimates
    .sort((a, b) => a.minutes - b.minutes)
    .slice(0, 4) || [];

  return (
    <Sheet isOpen={isOpen} onClose={onClose} snapPoints={[0.4, 0.6]} initialSnap={0}>
      <Sheet.Container>
        <Sheet.Header />
        <Sheet.Content>
                      ...
                      {limitedEstimates.map((estimate, idx) => (
                        <div key={idx} className="stop-sheet-estimate-item">
                          ...
                          <div className="stop-sheet-estimate-details">
                            <div className="stop-sheet-estimate-route">{estimate.route}</div>
